      ...
      axios
        .get(
          searchCars +
            ...
            "&model=" +
            this.state.search_model
        )
        .then((resData) => {
          if (resData.data == null || resData.data == "")
            return alert("No Cars Found");
      ...
      <PrimaryTemplate>
        <section className="feature-area">
          <section className="feature-text">
            ...
            <p>
              Are you looking for cheap car rental deals? We beleive in
              providing the highest level of service <br /> at the lowest
              possible price. Search right here!
            </p>
          </section>
          <br />
        ...
        <section className="homepage-content-container container">
          <div className="content-title">
            <Typography variant="h5" className="homepage-conent-heading">
              {" "}
              Latest Rental Car Offers{" "}
            </Typography>
          ...
          >
            {this.state.carsData
              ...
              .map((newData) => console.log(newData))}
            {this.state.searchCarsData.length === 0 ? (
              this.state.carsData
                .filter((fil) => fil.status == "Active")
                .map((cars) => (
                  <Grid
                    key={cars._id}
                    item
                    sm={6}
                    md={3}
                    onClick={() => this.productInfo(cars._id)}
                  >
                    <CarCard
                      carTitle={cars.name}
